...
import { DEFAULT_VIDEO_CONFIG, DEFAULT_RECORDING_CONFIG, FALLBACK_RECORDING_CONFIG } from "@/constants";
...
export const setupMediaRecorder = (stream: MediaStream) => {
  ...
  if (!isCodecSupported(config.mimeType)) {
    config = FALLBACK_RECORDING_CONFIG;
    if (!isCodecSupported(config.mimeType)) {
      // As a last resort, use generic webm (with bitrates)
      config = {
        mimeType: "video/webm",
        audioBitsPerSecond: DEFAULT_RECORDING_CONFIG.audioBitsPerSecond,
        videoBitsPerSecond: DEFAULT_RECORDING_CONFIG.videoBitsPerSecond,
      };
    }
  }
...
export const setupRecording = (
  ...
  if (!isCodecSupported(config.mimeType)) {
    config = FALLBACK_RECORDING_CONFIG;
    // If fallback is not supported, use generic webm (with same bitrates)
    if (!isCodecSupported(config.mimeType)) {
      config = {
        mimeType: 'video/webm',
        audioBitsPerSecond: DEFAULT_RECORDING_CONFIG.audioBitsPerSecond,
...
export const createIframeLink = (videoId: string) =>
  `https://iframe.mediadelivery.net/embed/450808/${videoId}?autoplay=true&preload=true`;
